...
First, we need an Apollo Graph Manager API key. Navigate to [Apollo Graph Manager](https://engine.apollographql.com/), login, and click on _New Graph_ on the sidebar or at the top. Step 1 of the the prompt will instruct you to name your graph. Step 2 will give you instructions on how to publish your schema. Follow the instruction. When you're finished, click _Done_. You will to find the API key by opening the Settings page, under API Keys. It will be prefixed by `service:`. Copy that key so we can save it as an environment variable.
